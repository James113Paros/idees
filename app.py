...
# Για αυτές τις κατηγορίες το "done" εξαιρεί από τυχαία επιλογή
DONE_EXCLUDES_RANDOM = {"volta", "drasthriothta", "meros"}
...
def random_idea():
    ...
    with get_conn() as conn:
        with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
            if category in DONE_EXCLUDES_RANDOM:
                cur.execute("SELECT * FROM ideas WHERE category=%s AND done=FALSE", (category,))
            else:
                cur.execute("SELECT * FROM ideas WHERE category=%s", (category,))
            ideas = cur.fetchall()
            if not ideas:
...
def mark_done(idea_id):
    with get_conn() as conn:
        with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
            cur.execute("UPDATE ideas SET done=NOT done WHERE id=%s RETURNING *", (idea_id,))
            conn.commit()
            return jsonify(cur.fetchone())
